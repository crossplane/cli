...
# The Renovate container can't run Nix sandboxed, so HOME=/homeless-shelter on
# the real filesystem. Any build that writes to $HOME creates that directory,
# and Nix then refuses to start another build until it's gone.
