...
func (m *Manager) GenerateFromMultipleSources(ctx context.Context, sources []Source) error {
	...
	if err := m.ensureLanguageDirsExist(); err != nil {
		return err
	}
...
// ensureLanguageDirsExist creates an empty directory for any configured
// language that produced no output this pass, so mergedSourcesFresh's
// DirExists check doesn't mistake "generated nothing" for "never generated".
func (m *Manager) ensureLanguageDirsExist() error {
	for _, lang := range m.languages() {
		if err := m.fs.MkdirAll(lang, 0o750); err != nil {
			return errors.Wrapf(err, "failed to ensure %s schema directory exists", lang)
		}
	}
	return nil
}
