...
```bash
# Ensure the locally-built crossplane binary is used
...
TypeScript schema generation starts a Docker container running a Node.js
toolchain, so unlike Go, JSON, KCL and Python it is not generated by default:
add `typescript` to `spec.schemas.languages` to opt in. Edit
`crossplane-project.yaml` to add it, along with dependencies:
...
`spec.schemas.languages` is an explicit list: it restricts generation to exactly what
it names, so add any other languages the project already generates alongside
`typescript`.
...
A `k8s` dependency's types are described by an OpenAPI spec rather than by
CRDs. The CLI downloads the upstream Kubernetes OpenAPI v3 documents for that
version and generates TypeScript models from them into your project's
`crossplane-models` package, alongside the models generated from CRDs:
...
Currently supported API groups are `core` (imported without a group prefix,
e.g. `crossplane-models/v1`), `apps`, `autoscaling`, `batch`,
`networking.k8s.io`, `policy`, `rbac.authorization.k8s.io`, and
`storage.k8s.io`.
...
generated. Unlike Go, JSON, KCL and Python, TypeScript is not generated unless
`spec.schemas.languages` names it explicitly — an omitted `languages` list, or
one that doesn't include `typescript`, produces no TypeScript models.
