...
// crdDocuments returns the CRD documents obj represents, keyed by the file
// name to write each one under: itself, if it's already a
// CustomResourceDefinition, or the CRD(s) Crossplane derives from it, if
// it's a CompositeResourceDefinition. It returns no documents (and no
// error) for any other kind of object.
func crdDocuments(obj runtime.Object) (map[string]runtime.Object, error) {
	switch c := obj.(type) {
	case *apiextv1.CustomResourceDefinition:
		return map[string]runtime.Object{crdFilename(c.Spec.Names.Plural, c.Spec.Group): c}, nil
...
// generates for an XRD via crd.FromXRD, keyed by the file to write each one
// under.
func xrdCRDDocuments(xrd *xrdv1.CompositeResourceDefinition) (map[string]runtime.Object, error) {
	xr, claim, err := crd.FromXRD(xrd)
	if err != nil {
		return nil, err
	}
	docs := map[string]runtime.Object{crdFilename(xr.Spec.Names.Plural, xr.Spec.Group): xr}
	if claim != nil {
		docs[crdFilename(claim.Spec.Names.Plural, claim.Spec.Group)] = claim
	}
	return docs, nil
}
